		...
		try {
			...
			while (goIntoLoop) {
				...
				if (request.startsWith("/all")) {
					...
					if (firstSpace != -1 && exists(otherPlayerId) && !getClient(otherPlayerId).inGame) { // run game
																											// logic to
																											// whoever
																											// was
																											// challenged
						if (this.name.equals(otherPlayerId)) {
							out.println("You can't play yourself! Better luck next time.."); //when tested we're not sure why it still passes into the else
						} else {
							this.inGame = false;
...
//	}
	...
	private boolean exists(String user) {
		...
		if (!doesExist)
			out.println(user + " does not exist in the server.");
		return doesExist;
	}
